...
import { ShoppingCart, Plus, Eye, Flame, Search } from "lucide-react";
...
// Additional images for existing items
import chickenStew from "@/assets/menu/chicken-stew.jpg";
import vegetableStew from "@/assets/menu/vegetable-stew.jpg";
import chickenAlfredo from "@/assets/menu/chicken-alfredo.jpg";
import carbonara from "@/assets/menu/carbonara.jpg";
import spaghettiBolognese from "@/assets/menu/spaghetti-bolognese.jpg";
import papPlain from "@/assets/menu/pap-plain.jpg";
import papVleis from "@/assets/menu/pap-vleis.jpg";
import papWors from "@/assets/menu/pap-wors.jpg";
import meatPie from "@/assets/menu/meat-pie.jpg";
import sausageRoll from "@/assets/menu/sausage-roll.jpg";
import vetkoek from "@/assets/menu/vetkoek.jpg";
  ...
  useEffect(() => {
    const loadMenu = async () => {
      ...
      try {
        ...
        console.log("[Menu] Categories:", catsResult.data?.length, "error:", catsResult.error);
        console.log("[Menu] Products:", prodsResult.data?.length, "error:", prodsResult.error);

        if (catsResult.error) {
          console.error("[Menu] Failed to load categories:", catsResult.error);
          toast.error("Failed to load menu categories");
        }
        if (prodsResult.error) {
          console.error("[Menu] Failed to load products:", prodsResult.error);
          toast.error("Failed to load menu items");
        }

        if (catsResult.data) {
          setCategories(catsResult.data);
          if (catsResult.data.length > 0 && !activeCategory) {
            setActiveCategory(catsResult.data[0].id);
          }
        }
        if (prodsResult.data) {
          ...
          await Promise.all(
            prodsResult.data.map(async (product) => {
          ...
          <>
            ...
            {filtered.length === 0 && (
              <div className="text-center py-12 text-muted-foreground">
                ...
                <p>{searchQuery ? `No dishes found for "${searchQuery}"` : "No products in this category yet"}</p>
              </div>
            )}
          </>
        )}
